...
import { fetchDashboard, fetchReportExtras } from "@/data.functions";
 ...
 * 粉丝质量（画像聚合）、影响力与声量 Top。入口在首页底部与日报页。
 */
export const Route = createFileRoute("/report")({
  loader: async () => {
    const [stats, extras] = await Promise.all([fetchDashboard(), fetchReportExtras()]);
    return { stats, extras };
  },
  head: () => ({
    meta: [
...
function ReportPage() {
  const { stats, extras } = Route.useLoaderData();
