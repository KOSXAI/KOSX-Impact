...
| `npm run check` | 名册校验 + typecheck + 测试（CI 同款检查） |
...
4. 配置数据源密钥：`npx wrangler secret put SOCIALDATA_API_KEY`
5. `npm run deploy` 部署 Worker（Cron Trigger 随 Worker 一起部署）
6. 可选：绑定自定义域名

GitHub Actions 自动部署：在仓库 Secrets 中配置 `CLOUDFLARE_API_TOKEN`（Workers 编辑权限）和 `CLOUDFLARE_ACCOUNT_ID`，之后 push 到 `main` 会自动应用迁移并部署（Worker Secrets 不受影响）。
